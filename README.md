...
- **Colors**: Recolours the match screen. Everything starts at autodarts' own colours, so switching it on changes nothing until you pick something
  ...
  - **Background**: the page behind the match, in autodarts' own midnight, one of seven dark pairs (*Royal*, *Forest*, *Petrol*, *Wine*, *Plum*, *Ember* and *Graphite*), or two colours of your own. The bottom bar and its buttons follow it: the bar a shade lighter than the page, and undo, *Next*, the input picker and the camera button beside the board in the page's colour, their labels as readable as on autodarts' own blue. Once a leg is won, the border that spins round the bar turns in lighter shades of it autodarts' mark stays on it, tinted to go with the colours
  ...
  - **More colours**: the other players' cards and the throw bar, the text on the cards and in the throw bar, and the bottom bar that holds undo and Next, each with a button back to autodarts' own. A bottom bar colour picked here wins over the background, and the bar's buttons then follow it instead
...
- **Streaming Mode**: Replaces the whole match screen with a broadcast overlay — a chroma key background, the dartboard, and a scoreboard you place where you want it
  ...
  - **Design**: two scoreboards to choose from, switchable mid-match
    - **Classic** (default): white rows on a black grid, cyan for whatever is live
  ...
  - **Averages** (optional): leg / set / match average beside each name; the Autodarts design shows the narrowest one the match has reached
  ...
  - Nothing else the extension draws appears over the overlay — takeout notices, replays and corrections all stay behind it, so they cannot darken the chroma key mid-stream
...
- **Darts Zoom**: A close-up of where each dart of the current visit landed, one tile per dart
  - Three positions. **Bottom** (default) gives each dart a third of the window along the foot of the screen, and moves autodarts' undo and next buttons up to the free space in the top right so they are not covered; **Top** leaves those buttons alone and puts the same strip under the throw display, as wide as it is — and it follows the throw display when the site changes layout
    - In the **Bottom** position that bar can be dragged anywhere in the window — the top right is only where it starts, and what is free space on one screen is the scoreboard on another. Grab it anywhere, buttons included: it stays a normal press until you actually move, so undo and Next go on working. Where you leave it is remembered, and it is always kept fully on screen, so a spot chosen on a large monitor is pulled back into view on a smaller one rather than lost off the edge. *Reset bar position* in the settings puts it back in the corner
  - **On Board** adds nothing to the screen at all: autodarts' own board — the camera view, or the vector board when no camera is running — zooms in on each dart as it lands and pulls back out again
    ...
    - Only a dart that has just been thrown moves the board — one your board or Lens detected, or a bot's. Correcting one, with autodarts' own correction or **Quick Correction**, taking one back, or entering one by hand — clicked onto the board or typed in on the keypad, as when the board missed it — leaves it where it is: a zoom is how you know a dart landed, so it should not go off while you are fixing the score
...
- **Board View**: Start every game — bull-off included — showing the camera, or the drawn board, you actually want
  ...
  - It waits for a button that can actually be pressed. While a bot, or a seat scored by hand, is at the oche the button is disabled and blank — and blank is not the drawn board, however much it looks like it
  - It presses again whenever the site forgets the view: the screen being rebuilt after the bull-off, or the turn passing to a board the site has not been told about yet, both of which put camera 1 back on their own
...
- **Board Skins**: Play on autodarts' drawn dartboard in another design
  - Six to choose from, each previewed in the settings — three to a row, two on a phone: **Default**, the board autodarts draws today, left exactly as it is; **Classic**, the board autodarts drew before its rebuild; **qwellcode**, a board with lime-green rings; **Opal**, mother-of-pearl segments on plum in a copper ring; **Marble**, black and white marble veined with gold in a gilded rim; and **Sorbet**, lemon, coral and mint pastels on a lavender ring
...
- **Larger Player Names**: Increase the font size of player names for better visibility during matches
  - Both sizes are given in rem — 1 is the browser's base size, 16 pixels by default — and both work in every layout of the match screen, the sidebar of a narrower window included
