...
- Ported seven of the Matches features to the rebuilt site, and checked the other eleven one by one
  ...
  - **Hide Menu In Match** and **Checkout Guide** are not ported and no longer needed: the rebuilt match screen ships no navigation to hide, and it shows a checkout route beside every player's score already — verified per player and live, from the same `state.checkoutGuides` the extension reads, with its own *Show checkout guide* switch. v1's version was in fact the weaker of the two: it only ever wrote the route of whoever was throwing, so the other cards kept a stale one until their turn came round again
  ...
  - **Zoom** is feasible without a board but not done yet
